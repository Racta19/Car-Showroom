    ...
    <div className='search-manufacturer'>
        <Combobox value={selected} onChange={setSelected}>
            <div className=' relative w-full'>
                ...
                <ComboboxInput className="search-manufacturer__input" placeholder='Volkswage' displayValue={(manufacturer: string) => manufacturer} onChange={(e) => setQuery(e.target.value)} />
                                ...
                                <ComboboxOption key={item} className={({ focus }) => `relative search-manufacturer__option ${ focus ? 'bg-primary-blue text-white' : 'text-gray-900'}`} value={item}>
                                    {({selected, focus}) => (
                                        <>
                                            ...
                                            { focus ? (<span className={`absolute inset-y-0 left-0 flex items-center pl-3 ${ focus ? 'text-white': 'text-teal-600'}`}></span>) : (null)}
                                        </>
                                    )}
                                </ComboboxOption>
                            ))
